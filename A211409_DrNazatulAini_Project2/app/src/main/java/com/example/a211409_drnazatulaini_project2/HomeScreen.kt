...
@Composable
fun HomeScreen(onNavigateToScaler: () -> Unit, onNavigateToExtra: () -> Unit, onNavigateToScanner: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize().padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
        ...
        Spacer(modifier = Modifier.height(16.dp))
        Button(onClick = onNavigateToScanner, modifier = Modifier.fillMaxWidth().height(56.dp), colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.tertiary)) {
            Text("Scan Barcode (Camera API)")
        }
    }
}